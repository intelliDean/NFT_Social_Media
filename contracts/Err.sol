// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;

contract Err {

    error ADDRESS_ZERO_NOT_ALLOWED();
    error YOU_ARE_NOT_A_USER__GO_REGISTER();
    error ONLY_OWNER();
    error ONLY_USER();
    error ALREADY_REGISTERED__LOGIN();
    error YOU_ARE_NOT_LOGGED_IN__LOG_IN_TO_CONTINUE();
    error COULD_NOT_CREATE_NFT();
    error YOU_ARE_NOT_A_MEMBER_OF_THIS_GROUP();
    error CONTENT_TOO_LONG();
    error NAME_TOO_LONG();
    error NAME_OR_USERNAME_CANNOT_EMPTY();
    error URI_CANNOT_BE_EMPTY();
    error GROUP_NAME_CANNOT_BE_EMPTY();
    error CONTENT_CANNOT_BE_EMPTY();
    error NAME_CANNOT_BE_EMPTY();
}
